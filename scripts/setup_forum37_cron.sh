...
Does not modify daily_run slots installed by scripts/setup_cron.sh.

Usage:
  ./scripts/setup_forum37_cron.sh [--dry-run] [--repair-crontab] [--remove]
...
install_user_crontab_line() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    if is_forum37_cron_line "$line"; then
      continue
    fi
    printf '%s\n' "$line"
...
line="$(cron_line_for_forum37 "$TIME")"

if [[ "${REMOVE:-0}" == 1 ]]; then
  install_user="$(resolve_cron_install_user)"
  if [[ "${DRY_RUN:-0}" == 1 ]]; then
    echo "[dry-run] would remove forum37 cron line for $install_user"
    exit 0
  fi
  install_user_crontab_line "$install_user" "" 1
  echo "[ok] forum37 cron removed for $install_user"
  restart_cron_daemon || true
  exit 0
fi

if [[ "${DRY_RUN:-0}" == 1 ]]; then
  echo "[dry-run] would install forum37 cron line:"
  echo "  $line"
