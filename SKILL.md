...
Send three interactive cards (scan summary / download fail / download success):
...
`daily_run.py` auto-sends cards when `FEISHU_RECEIVE_ID` is set (use `--no-feishu` to disable).
