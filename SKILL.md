...
**Forum-142 batch (daily 14:00 Beijing):** `./scripts/setup_forum142_cron.sh` installs `./scripts/forum142_batch_run.sh` (10-page batch scan from page 200, log `data/forum142_batch_run.log`, per-run report `reports/forum-142_YYYY-MM-DD.md`). Cron-only; deploy on cron VM after merge.
