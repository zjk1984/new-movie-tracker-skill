...
A Python script fetches curated RSS feeds across six categories (Chinese/English tech, politics & economics, AI, insights) and pushes new items to a Feishu group. The entrypoint remains `scripts/cnbeta_rss.py` for backward compatibility with existing cron wrappers.
...
flowchart LR
  ...
  Limit --> Update[update/YYYYMMDD-HHMMSS.md]
  Limit --> Feishu[Feishu app bot or webhook]
...
8. **Send** grouped by category to Feishu as an interactive card (or plain text). If nothing qualifies, log `no new RSS items`, skip Feishu, and do not write markdown.
9. **Archive** each non-empty batch to `update/YYYYMMDD-HHMMSS.md` with `<!-- category: ... -->` tags; move the previous file to `update/backup/`.
...
| Path | Role |
|------|------|
| `scripts/cnbeta_rss.py` | Multi-source fetch, parse, dedupe, notify |
...
| `tests/test_cnbeta_rss.py` | RSS parsing, limits, dedup, and message tests |
